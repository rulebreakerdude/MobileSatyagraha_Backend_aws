...
#****************************************************************************		
	...
	def getCGSwaraUnsyncedNumberData(self):
		pingAndReconnect(self)
		z='{:%Y%m%d}'.format(datetime.datetime.now())
		z=z+'000000'
		self.c.execute("SELECT ref_id,phoneNumber FROM CGSwara_IMI_call_log WHERE is_synced = 0 and datetime_of_api_call >= %s;",(z,))
		db_response=self.c.fetchall()
		return db_response
		
	def setCGSwaraSyncedNumberData(self,ref_id):
		pingAndReconnect(self)
		self.c.execute("UPDATE CGSwara_IMI_call_log SET is_synced = 1 WHERE (ref_id = %s);",(ref_id,))
		self.conn.commit()
#****************************************************************************
